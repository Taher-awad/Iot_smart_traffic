...
aws iot create-keys-and-certificate \
    --set-as-active \
    --certificate-pem-outfile ../config/certificate.pem.crt \
    --public-key-outfile ../config/public.pem.key \
    --private-key-outfile ../config/private.pem.key > ../config/cert_output.json

# Extract Certificate ARN for attachment
CERT_ARN=$(jq -r '.certificateArn' ../config/cert_output.json)
...
curl -o ../config/root-CA.crt https://www.amazontrust.com/repository/AmazonRootCA1.pem
